...
/**
 * Encoded Read-only map implementation.
 *
 * @since 9.2
 */
@Experimental
public final class EncodedReadOnlyMapImpl<K, V> extends AbstractFunctionalMap<K, V> implements ReadOnlyMap<K, V> {
   ...
   private EncodedReadOnlyMapImpl(Params params, FunctionalMapImpl<K, V> functionalMap) {
   ...
   private static <K, V> ReadOnlyMap<K, V> create(Params params, FunctionalMapImpl<K, V> functionalMap) {
      ...
      EncodedReadOnlyMapImpl readOnlyMap = new EncodedReadOnlyMapImpl<>(params, functionalMap);
      ...
      readOnlyMap.cacheEncoders = new CacheEncoders(keyEncoder, keyWrapper, valueEncoder, valueWrapper);
      readOnlyMap.encodingClasses = encoderCache.getEncodingClasses();
      return readOnlyMap;
   }
   ...
   public <R> CompletableFuture<R> eval(K key, Function<ReadEntryView<K, V>, R> f) {
      ...
      ReadOnlyKeyCommand cmd = fmap.commandsFactory.buildReadOnlyKeyCommand(keyEncoded, (Function) f, params, encodingClasses);
   ...
   public <R> Traversable<R> evalMany(Set<? extends K> keys, Function<ReadEntryView<K, V>, R> f) {
      ...
      ReadOnlyManyCommand cmd = fmap.commandsFactory.buildReadOnlyManyCommand(encodedKeys, (Function) f, params, encodingClasses);
   ...
   public Traversable<ReadEntryView<K, V>> entries() {
      ...
      return Traversables.of(stream.map(e -> EntryViews.readOnly(e, cacheEncoders)));
   }
