...
import org.infinispan.query.objectfilter.impl.ql.*;
}
...
joins
	:	PROPERTY_JOIN jt=joinType ft=FETCH? an=ALIAS_NAME pf=PROP_FETCH?
		{ delegate.activateFromStrategy($jt.jt, $ft, $pf, $an); }
...
joinType returns [JoinType jt]
	:	CROSS { $jt = JoinType.CROSS; }
	|	INNER { $jt = JoinType.INNER; }
	|  LEFT { $jt = JoinType.LEFT; }
	|  RIGHT { $jt = JoinType.RIGHT; }
	|  FULL { $jt = JoinType.FULL; } OUTER?
	;
...
propertyReferenceExpression
   :  propertyReference
   |  PATH ret=propertyReferencePath PATH<node=PropertyPathTree> //TODO WHAT IS THIS [$PATH, $ret.retPath] propertyReferencePath
   ;
...
propertyReference
	:	PROPERTY_REFERENCE ret=propertyReferencePath PROPERTY_REFERENCE<node=PropertyPathTree> //TODO WHAT IS THIS [$PROPERTY_REFERENCE, $ret.retPath] propertyReferencePath
	;
